...
export const POST = async (request: Request): Promise<Response> => {
  ...
  if (origin && !allowedOrigins().has(origin)) {
    return Response.json({ message: 'Origin not allowed.' }, { headers, status: 403 })
  }
  ...
  const rate = !previous || previous.expires <= now
  ...
  if (rate.count > MAX_ATTEMPTS) {
    return Response.json({ message: 'Too many requests. Please try again later.' }, { headers, status: 429 })
  }
  ...
  try {
    ...
    const quotePage = await payload.findGlobal({ slug: 'quote-page', overrideAccess: true })
    const values = (options: typeof quotePage.helpTypes) =>
      (options || []).map((option) => option.value).filter(Boolean)
    const input = parseQuoteRequest(body, {
      helpType: values(quotePage.helpTypes),
